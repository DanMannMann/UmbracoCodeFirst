...
{
    public abstract class ListViewDocumentType : ListViewDocumentType<DocumentTypeBase>
    {
        ...
        private void init()
        {
            if (_children == null)
            {
                ...
                if (NodeDetails.IsPublishedInstance)
                {
                    ...
                    {
                        Type docType;
                        if (DocumentTypeRegister.DocumentTypeCache.TryGetValue(child.DocumentTypeAlias, out docType))
                        {
                            MethodInfo convertToModel = DocumentTypeRegister.EnsureRegisterConvertMethod(docType);
                            var stronglyTypedModel = convertToModel.Invoke(null, new object[] { child });
                            if (stronglyTypedModel is T)
                            {
                                _children.Add(stronglyTypedModel as T);
                            }
                        }
                    }
                }
                else
                {
                    ...
                    {
                        Type docType;
                        if (DocumentTypeRegister.DocumentTypeCache.TryGetValue(child.ContentType.Alias, out docType))
                        {
                            MethodInfo convertToModel = DocumentTypeRegister.EnsureRegisterConvertMethod(docType);
                            var stronglyTypedModel = convertToModel.Invoke(null, new object[] { child });
                            if (stronglyTypedModel is T)
